...
def load_and_filter(file_path: Path) -> mne.io.Raw:
    ...
    print(
        f"    channels={len(raw.ch_names)}, sfreq={raw.info['sfreq']}, duration={raw.times[-1]:.0f}s"
    )
    ...
    raw.filter(
        FREQ_BANDS["full"][0], FREQ_BANDS["full"][1], fir_design="firwin", verbose=False
    )
...
def extract_events(
    raw: mne.io.Raw, dataset: str = "auto"
    ...
    if detected is not None:
        ...
        label_map = None
        print(
            f"    Events (unknown dataset): {len(events)} — unique IDs={sorted(unique_ev)}"
        )
...
def main():
    ...
    parser.add_argument(
        "--input", default="data/raw/physionet_mi", help="Input directory"
    )
    parser.add_argument("--output", default="data/processed", help="Output directory")
    parser.add_argument(
        "--max-channels", type=int, default=64, help="Cap channels to first N"
    )
    parser.add_argument(
        "--channels",
        default="motor8",
        choices=["motor8", "motor16", "all"],
    ...
    for i, fp in enumerate(files):
        ...
        result = process_subject(
            fp, channel_picks, dataset=args.dataset, tmin=args.tmin, tmax=args.tmax
        )
        if result is None:
            continue
        X, y = result
    ...
    # NOTE: this assumes the canonical PhysioNet MI convention
    # (0=rest, 1=left, 2=right). It is INVALID for BCI IV 2a (0=left), where it
    # would silently drop the left-hand class — guard against that here.
    if args.binary and args.dataset == "bci_iv_2a":
        parser.error(
            "--binary is only valid for PhysioNet MI (label 0 = rest). "
            "BCI IV 2a uses 0=left; use the 4-class pipeline "
            "(preprocessing/prepare_bci_iv_2a.py) instead."
        )
    if args.binary:
        mask = y_all != 0
        X_all = X_all[mask]
    ...
    # Train/val split — 75/25 trial-level stratified split on the AGGREGATED
    # multi-subject data. NOTE: this is subject-MIXED, not subject-independent —
    # the same subject's trials land in both train and val. Use this path for
    # quick debugging only; run LOSO (training/train_loso.py) for a valid
    # cross-subject generalization estimate.
    try:
