...
> 提示：如果 `deepbci_stream.py` 找不到设备，检查设备管理器中 USB 串口号，在脚本里改 `PORT` 变量。

**纯软件环境验证（不需硬件）：**

```bash
python main.py demo         # 合成数据跑通整条管线
pytest tests/ -q            # 确认 745 个测试通过
```
...
| SPDNet + EA (d=[8,8], seed123) | **59.56%** ± 8.56% | 0.190 |
| SPDNet + EA (d=[8,8], seed456) | **61.48%** ± 9.02% | 0.225 |
...
| EEG-Conformer ✓ | 61.33% | 64.22% | +2.89pp | LayerNorm + 残差 |
| EEG-TCNet ✓ | 61.56% | 63.85% | +2.29pp | BatchNorm + 残差 |
| Tangent Space | 60.44% | 60.44% | ±0.00pp | 仿射不变 |

> ✓ = 3-seed 均值（seed 42/123/456）。Conformer + EA 均值 64.22%、TCNet + EA 均值 63.85%。

**机制**: EA 将跨被试 Riemannian 协方差距离减少 **85.1%**（12–16Hz 频带最稳定）。FBCNet 的方差池化层和 SPDNet 的 SPD 流形层直接读取协方差结构，因此受益最大；TCN/Transformer 的内部归一化（LayerNorm/BatchNorm + 残差）已消除大部分分布偏移，EA 边际增益小。
...
| 硬件通道 | 电极位置 | 10-20 位置 | MI 作用 |
|----------|----------|-----------|---------|
| Ch1 | FC3 | 左前运动区 | 运动准备辅助 |
| Ch2 | **C3** | 左中央区 | ★ **右手 MI 关键** |
| Ch3 | Cz | 中央中线 | 参考/基线 |
| Ch4 | **C4** | 右中央区 | ★ **左手 MI 关键** |
| Ch5 | FC4 | 右前运动区 | 运动准备辅助 |
| Ch6 | CP3 | 左中央-顶叶 | 体感反馈 |
| Ch7 | CPz | 中央-顶中线 | 体感参考 |
| Ch8 | CP4 | 右中央-顶叶 | 体感反馈 |
...
> C3/C4 是运动想象最关键的通道。想象右手 → 左脑 C3 活跃（μ 波下降）；想象左手 → 右脑 C4 活跃。其他 6 个通道提供空间上下文。
...
| 目录 | 干嘛的 | 什么时候用 |
|------|--------|------------|
| `models/` | 所有模型定义（EEGNet ↔ BRT-Det） | 要改网络结构、加新模型 |
| `training/` | 训练 + LOSO 脚本 | 要跑实验、调超参 |
| `preprocessing/` | MNE 预处理 + EA 对齐 | 处理新数据集 |
| `features/` | CSP / Riemannian 特征提取 | 跑传统基线 |
| `realtime/` | 实时推理 + LSL + DeepBCI 连接 | 部署到树莓派 / 在线演示 |
| `scripts/` | 分析、可视化、论文图表 | 画结果图、消融分析 |
| `ui/` | Streamlit 实时看板 | 浏览器看波形 |
| `data/` | 数据下载、存储 | 换硬盘路径 |
| `tests/` | 745 个单元测试 | 改完代码跑 `pytest` |
| `docs/` | 开发日志、论文草稿、技术报告 | 写论文、查实验记录 |
...
# 按提示佩戴电极 → 静息基线 → 左手想象 → 右手想象

# 导出为模型训练格式
python datasets/deepbci_loader.py -i data/subjects/sub_001/ --all
# 输出: X.npy [N, 8, 750], y.npy [N]

# 用自己数据微调预训练模型
python main.py loso --data_dir data/subjects/ --finetune 10
```
